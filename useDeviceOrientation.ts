...
export const useDeviceOrientation = (config: DeviceOrientationConfig = {}) => {
  ...
  const isMobileAndPortrait = computed(
    () => isMobile.value && isPortrait.value,
  );

  const orientationLoading = ref(false);

  // Wait for orientation changes and trigger callback
  const onOrientationChange = async (cb?: () => void) => {
